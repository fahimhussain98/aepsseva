...
public class PayspirntSettlementActivity extends AppCompatActivity {
    ...
    private void getAccountDetails() {
        ...
        Call<JsonObject> call = ApiController.getInstance().getApi().getAccountDetailsPaySprint(ApiController.Auth_key, userId, deviceId, deviceInfo);
        call.enqueue(new Callback<JsonObject>() {
            @Override
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                if (response.isSuccessful()) {
...
*/
       ...
       /* Call<JsonObject> call = ApiController.getInstance().getApiSecond().moveToBank(ApiController.Auth_key, userId, deviceId, deviceInfo, selectedPaymentType, amount,
                selectedBeneId, selectedIfscNumber, selectedAccountType, selectedTransactionType, selectedAccountHolderName, "2270"); */

        // below ADDED for paysprint settlemnet Activity BACKUP FROM PAYET SuperAdmin //

        Call<JsonObject> call = ApiController.getInstance().getApiSecond().paySprintMoveToBank(ApiController.Auth_key, userId, deviceId, deviceInfo, selectedPaymentType, amount,
                selectedAccountNumber, selectedBeneId, selectedIfscNumber, selectedAccountType, selectedTransactionType,selectedAccountHolderName,selectedBankName, "2270");
